...
from pyciemss.utils import get_tspan, reparameterize
...
class TestPetrinetDerivatives(unittest.TestCase):
    ...
    def setUp_MIRA(self):
        ...
        self.initial_state = dict(
            susceptible_population=3.0, infected_population=1.0, immune_population=0.0
        )

        # Initialize
        self.MIRA_model = setup_model(
            self.petri, start_time=0, start_state=self.initial_state
        )
    ...
    def setUp_Hand(self):
        ...
        self.raw = SIR_with_uncertainty(
            N=100.0, beta=0.1, gamma=0.2
        )  # N here unecessary, makes its own
        # self.assertIsInstance(self.raw, "x") # test that it is "x"

        # initial_state is the same

        # Initialize
        self.Hand_model = setup_model(
            self.raw, start_time=0, start_state=self.initial_state
        )
    ...
    def test_derivs(self):
        ...
        for i in range(nsamples):
            hand_model = reparameterize(
                self.Hand_model,
                {"beta": prior_samples["beta"][i], "gamma": prior_samples["gamma"][i]},
            )
            ...
            for trajectory in prior_samples:
                if "_sol" in trajectory:
                    self.assertTrue(
                        torch.allclose(
                            prior_samples[trajectory][i], trajectories[trajectory][0]
                        )
                    )
