...
class TestObservables(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sidarthe = load_petri_model(
        ...
        tm.observables = {
            "half_population": Observable(
                name="half_population", expression=SympyExprStr(expr / 2)
            )
        }
    ...
    def test_observables(self):
        ...
        sir = setup_petri_model(
            raw_sir,
            0.0,
            dict(
                susceptible_population=1000.0,
        ...
        observations = solutions_to_observations(
            timepoints, sir_sample_df.set_index(["timepoint_id", "sample_id"])
        )
        ...
        for timepoint, data in sir_data:
            data["half_population"] = (
                data["immune_population"]
                + data["susceptible_population"]
                + data["infected_population"]
            ) / 2
        inferred_parameters = calibrate(sir, sir_data, num_iterations=10)
        self.assertTrue(inferred_parameters)
        calibrated_samples = sample_petri(
            sir, timepoints, num_samples, inferred_parameters
        )
    ...
    def test_observables_actually_calibrate(self):
        ...
        sidarthe_samples = load_and_sample_petri_model(sidarthe_mira, 1, timepoints)[
            "data"
        ].set_index(idx)
