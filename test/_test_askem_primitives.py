...
from pyciemss.ODE.askem_primitives import (
    sample,
    infer_parameters,
    intervene,
    optimization,
...
class TestDensityTest(unittest.TestCase):
    ...
    def test_december_demo_walkthrough(self):
        ...
        NUM_SAMPLES = 3
        INITIAL_STATE = tuple(
            torch.as_tensor(s) for s in (1 - 81 / 100000, 0.0, 81 / 100000, 0.0, 0.0)
        )
        TSPAN = get_tspan(1, 99, 99)

        prior_data_cube = sample(ode_model, NUM_SAMPLES, INITIAL_STATE, TSPAN)
        ...
        inferred_parameters = infer_parameters(
            ode_model,
            NUM_ITERATIONS,
            HIDDEN_OBSERVATIONS,
            DATA,
        ...
        INTERVENTION = constant_intervention_builder(
            "SV_flux", torch.tensor(0.0), TSPAN
        )
        ...
        solution = optimization(
            INITIAL_GUESS, OBJECTIVE_FUNCTION, CONSTRAINTS, OPTIMIZER
        )
