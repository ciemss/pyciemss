...
class TestVega(unittest.TestCase):
    ...
    def test_histogram_refs(self):
        for num_refs in range(1, 20):
            ...
            self.assertEqual(num_refs, len(by_key_value(hist["data"], "name", "xref")["values"]),
                             "Nonzero xrefs not as expected")
            self.assertEqual(num_refs, len(by_key_value(hist["data"], "name", "yref")["values"]),
    ...
    def test_histogram_multi(self):
        hist = vega.histogram_multi(s30=self.s30, r30=self.r30, i30=self.i30)
        data = pd.DataFrame(by_key_value(hist["data"], "name", "binned")["values"])
        self.assertEqual({"s30", "i30", "r30"}, set(data["label"].values))

        
        hist = vega.histogram_multi(s30=self.s30, r30=self.r30)
        data = pd.DataFrame(by_key_value(hist["data"], "name", "binned")["values"])
