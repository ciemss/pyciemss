...
class Test_Interface_Utils(unittest.TestCase):
    ...
    def test_convert_to_output_format(self):
        ...
        assert_frame_equal(
            expected_output,
            convert_to_output_format(self.intervened_samples,
                                     self.timepoints,
                                     self.interventions),
    ...
    def test_intervention_to_interval(self):
        ...
        self.assertEqual(expected_intervals,
                         interventions_and_sampled_params_to_interval(self.interventions, self.sampled_params))
                         
    def test_assign_interventions_to_timepoints(self):
        """Test assign_interventions_to_timepoints."""
        expected_intervened_values = {
            'beta_param': [
                0.2, 0.2, 2.0, 2.0, 2.0, 0.25, 0.25, 2.0, 2.0, 2.0, 0.15, 0.15, 2.0, 2.0, 2.0
            ],
            'gamma_param': [
                0.3, 0.3, 0.3, 0.1, 0.1, 0.25, 0.25, 0.3, 0.1, 0.1, 0.35, 0.35, 0.3, 0.1, 0.1
            ]
        }
        self.assertEqual(expected_intervened_values, assign_interventions_to_timepoints(
