...
from pyciemss.ODE.interventions import (
    constant_intervention_builder,
    parameter_intervention_builder,
    state_dependent_intervention_builder,
    time_and_state_dependent_intervention_builder,
    time_dependent_intervention_builder,
)
...
class TestInterventions(unittest.TestCase):
    ...
    def test_constant_intervention_builder(self):
        ...
        intervention = constant_intervention_builder(
            "SV_flux", torch.tensor([1.0]), self.tspan
        )
        intervened_model = intervene(self.model, intervention)
        result = sample(
            intervened_model, self.num_samples, self.initial_state, self.tspan
        )
        self.assertIsNotNone(result)

    def test_parameter_intervention_builder(self):
        # Just testing whether the intervention builder constructs a model
        intervention = parameter_intervention_builder("nu", torch.tensor([1.0]))
        intervened_model = intervene(self.model, intervention)
        result = sample(
            intervened_model, self.num_samples, self.initial_state, self.tspan
        )
    ...
    def test_state_dependent_intervention_builder(self):
        # Just testing whether the intervention builder constructs a model
        intervention = state_dependent_intervention_builder(
            "SV_flux", lambda x: x * 0.3, self.tspan
        )
        intervened_model = intervene(self.model, intervention)
        result = sample(
            intervened_model, self.num_samples, self.initial_state, self.tspan
    ...
    def test_time_dependent_intervention_builder(self):
        # Just testing whether the intervention builder constructs a model
        intervention = time_dependent_intervention_builder(
            "SV_flux", lambda t: 1 / t, self.tspan
        )
        intervened_model = intervene(self.model, intervention)
        result = sample(
            intervened_model, self.num_samples, self.initial_state, self.tspan
    ...
    def test_time_and_state_dependent_intervention_builder(self):
        # Just testing whether the intervention builder constructs a model
        intervention = time_and_state_dependent_intervention_builder(
            "SV_flux", lambda t, x: x / t, self.tspan
        )
        intervened_model = intervene(self.model, intervention)
        result = sample(
            intervened_model, self.num_samples, self.initial_state, self.tspan
