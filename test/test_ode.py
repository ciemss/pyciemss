...
class TestODE(unittest.TestCase):
    ...
    def test_load_remove_start_event(self):
        ...
        self.assertEqual(
            self.model._static_events[0].initial_state["S"], torch.tensor(0.9)
        )
        self.assertEqual(
            self.model._static_events[0].initial_state["I"], torch.tensor(0.1)
        )
        self.assertEqual(
            self.model._static_events[0].initial_state["R"], torch.tensor(0.0)
        )
    ...
    def test_load_remove_observation_events(self):
        ...
        self.assertEqual(
            self.model._static_events[0].observation["S"], torch.tensor(0.9)
        )
        self.assertEqual(
            self.model._static_events[0].observation["I"], torch.tensor(0.1)
        )
        self.assertEqual(
            self.model._static_events[1].observation["S"], torch.tensor(0.8)
        )
    ...
    def test_observation_indices_and_values(self):
        ...
        self.assertTrue(
            torch.equal(self.model._observation_values["S"], torch.tensor([0.9, 0.8]))
        )
        self.assertTrue(
            torch.equal(self.model._observation_values["I"], torch.tensor([0.1]))
        )
