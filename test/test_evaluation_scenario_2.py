...
def mira_initial_state(SIDARTHE_model):
    return tuple(
        torch.as_tensor(v.data["initial_value"])
        if v.data["initial_value"] is not None
...
def initialize_MIRA_SIDARTHE_model(
    MODELPATH="models/evaluation_examples/scenario_2/",
...
def day_around_47(simulate_for_days):
    assert (
        torch.abs(
            torch.argmax(simulate_for_days["I_total_obs"]) / 10 - torch.tensor(47)
        )
        <= 1
    )
