...
def buffered_pof(samples, threshold=0.0):
    """
...
def alpha_superquantile(samples, alpha=0.95):
    """
    upper alpha-superquantile for a given alpha in [0,1] a.k.a conditional value-at-risk (CVaR),
    ...
    return qtile + 1.0 / (nsamples * (1 - alpha)) * np.sum(
        sorted_samples[0 : ka - 1] - qtile
    )
