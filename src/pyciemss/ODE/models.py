...
from pyciemss.ODE.base import PetriNetODESystem, ODE, Time, State, Solution, get_name
...
class MIRA_SIDARTHE(PetriNetODESystem):
    ...
    def observation_model(
        ...
        with pyro.condition(data=data if data is not None else {}):
            ...
            total_infections = (
                named_solution["Infected"]
                + named_solution["Diagnosed"]
                + named_solution["Ailing"]
                + named_solution["Recognized"]
                + named_solution["Threatened"]
            )
            return tuple(
    ...
    def __init__(self, G, *, noise_var: float = 1):
        ...
        for param in self.G.parameters.values():
            ...
            setattr(
                ...
                get_name(param),
                pyro.nn.PyroSample(
                    log_normal_transform(mu=prior_mean, sigma=prior_stdev)
                ),
            )
