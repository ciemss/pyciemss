...
def plot_predictive(
    ...
    **kwargs
    ...
    if vars[0] not in datacube and vars[0] == "I_obs":
        ...
        )  # TODO: This is too specific and needs to be changed
...
def plot_trajectory(
    ...
    ax.plot(
        tspan,
        datacube[vars[0]].squeeze().detach().numpy(),
        color,
        alpha=alpha,
        lw=lw,
        marker=marker,
...
def plot_ouu_risk(
    datacube,
    ax=None,
    xlabel: str = "7-day average infectious at 90 days",
    color: list = ["#377eb8", "#ff7f00", "#984ea3", "#ffd92f", "#a65628"],
    alpha=0.5,
    label: list = ["alpha-superquantile"],
    tmin=None,
):
    if not ax:
        ax = setup_ax()
