...
#     """
#     def root_id(id): return id[:-1]
#     G2 = nx.MultiDiGraph()
#     transitions = [n for n, d in bilayerG.nodes(data=True)
#                 if d["layer"] == "mediator"]

#     states = [*set([n for n, d in bilayerG.nodes(data=True)
#                 if d["layer"] == "input"]  +
#                 [root_id(n) for n, d in bilayerG.nodes(data=True)
#                 if d["layer"] == "output"] )]

#     inputs = [{"is": states.index(a)+1, "it": transitions.index(b)+1}
#             for a,b,d in bilayerG.edges(data=True)
#             if d["type"] == "input"]

#     outputs = [{"ot": transitions.index(a)+1, "os": states.index(root_id(b))+1}
#             for a,b,d in bilayerG.edges(data=True)
#             if d["type"] == "outcome"]
...
#     pG = petri_utils.load({"S": states, "T": transitions,
#                             "I": inputs, "O": outputs})
...
#     node_cmap = {"input": "slategray",
#                  "output": "green",
#                  "mediator": "mediumorchid"}

#     node_colors =[node_cmap[n]
#                   for n
#                   in nx.get_node_attributes(G, "layer").values()]
