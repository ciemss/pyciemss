...
from pyciemss.utils.complexvid_scabini.model import (
    isolate_node,
    remove_layer,
    include_layer,
    scale_layer,
...
def simulate(
    ...
    infected_t0=1,
    days=1,
    hospital_isolation="hospital - total",
    home_isolation="home - total",
):
    ...
    amostragem = np.random.lognormal(
        mean=1.621, sigma=0.418, size=1000
    )  # formato do tempo de incubacao (referencia no paper)
    prob_start_symptoms = (
        np.histogram(amostragem, bins=days_start_symptoms, density=True)
    )[0]
    ...
    severe_death_prob = (
        0.15 / 5
    )  # 15% de mortalidade dividida entre os dias 20-24 (5 dias)
    critical_death_prob = (
        0.5 / 4
    )  # 50% de mortalidade dividida entre os dias 21-24 (4 dias)
        ...
        try:
            while i0 < infected_t0:
                ...
                if grau[node] >= avg_k - i and grau[node] <= avg_k + i:
                    states[node] = all_states[
                        (random.choices(infected_states, prob_states, k=1))[0]
                    ]
    ...
    for day in range(0, days):
        ...
        for acao in parameters["acoes"]:
            ...
            if day == acao:
                ...
                changed = len(parameters["layers_por"][iii]) - len(
                    parameters["layers_tirar"][iii]
                )
                if changed > 0:
        ...
        for i in infected_list:
            ...
            if states[i] == all_states["infected - asymptomatic"]:
                ...
                if (
                    days_infected[i] >= 8
                ):  # asintomatico pode recuperar depois de 8 dias
                ...
                if days_infected[i] == days_symptoms[i]:
                    if flip_coin(0.2):
                        dayly_new_cases[
                            "diagnosed"
                        ] += 1  # considerando que apenas 20% desses se testam
                    else:
                        dayly_new_cases["undiagnosed"] += 1

                    G = isolate_node(
                        G, i, how=home_isolation
                    )  # se isolam em casa quando começam sintomas
                ...
                if days_infected[i] == days_symptoms[i]:
                    ...
                    dayly_new_cases[
                        "diagnosed"
                    ] += 1  # casos diagnosticados = severe e critical quando vao ao hospital
                    G = isolate_node(
                        G, i, how=hospital_isolation
                    )  # se isolam no hospital
        ...
        nx.set_node_attributes(
            G, days_infected, "days infected"
        )  # estado inicial de todos nodes eh suscetivel
        ...
        for i in infected_list:
            ...
            for edge in edges_i:
                ...
                # standard infection
                if (
                    states[edge[0]] >= all_states["infected - asymptomatic"]
                    and states[edge[0]] <= all_states["infected - critical"]
                ):  # se eh algum tipo de infectado
                    if (
                        all_states["susceptible"] == states[edge[1]]
                    ):  # se adjacente é suscetivel
                        if flip_coin(link["weight"]):  # tenta infectar
                            estado = (
                                random.choices(infected_states, prob_states, k=1)
                            )[0]
                            new_states[edge[1]] = all_states[
                                estado
                            ]  # determina aleatoriamente um tipo de caso
                            days_symptoms[edge[1]] = (
                                random.choices(
                                    days_start_symptoms, prob_start_symptoms, k=1
                                )
                            )[
                                0
                            ]  # determina quando os sintomas irao começar (ignora caso assintomatico)
                            dayly_new_cases[estado] += 1  # casos diarios
                            include_infected.append(edge[1])

                elif (
                    states[edge[1]] >= all_states["infected - asymptomatic"]
                    and states[edge[1]] <= all_states["infected - critical"]
                ):  # se eh algum tipo de infectado
                    if (
                        all_states["susceptible"] == states[edge[0]]
                    ):  # se adjacente é suscetivel
                        if flip_coin(link["weight"]):  # tenta infectar
                            estado = (
                                random.choices(infected_states, prob_states, k=1)
                            )[0]
                            new_states[edge[0]] = all_states[
                                estado
                            ]  # determina aleatoriamente um tipo de caso
                            days_symptoms[edge[0]] = (
                                random.choices(
                                    days_start_symptoms, prob_start_symptoms, k=1
                                )
                            )[
        ...
        count[5][day] = (
            dayly_new_cases["infected - asymptomatic"] + dayly_new_cases["undiagnosed"]
        )
