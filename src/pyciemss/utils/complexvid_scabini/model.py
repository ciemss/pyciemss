...
import pyciemss.utils.complexvid_scabini.comunities as communities
...
def createGraph(parameters):
    ...
    nx.set_node_attributes(
        G, ages, "age"
    )  # add o atributo age aos nos (index da faixa etaria)
    nx.set_node_attributes(
        G, states, "condition"
    )  # estado inicial de todos nodes eh suscetivel
    nx.set_node_attributes(
        G, days_infected, "days infected"
    )  # dias desde que o nó ta infectado
...
def include_layer(G, layer, parameters):
    ...
    if layers[layer] == 1:
        G = communities.generate_layer1(G, parameters)  # casas
    elif layers[layer] == 2:
        G = communities.generate_layer_school(G, parameters)  # escolas
    elif layers[layer] == 3:
        G = communities.generate_layer_church(G, parameters)  # templos
    elif layers[layer] == 4:
        G = communities.generate_layer_transport(G, parameters)  # transporte
    elif layers[layer] == 5:
        G = communities.generate_layer_random(G, parameters)  # aleatorio
    elif layers[layer] == 6:
        G = communities.generate_layer_work(G, parameters)  # trabalho
