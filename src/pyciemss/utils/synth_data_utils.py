...
def box_plot_weights(generating_weights, calibrated_solution):
    ...
    for i in range(num_models):
        plt.plot((i + 1), generating_weights[i], mfc=COLOR[i], mec='k', ms=7, marker="o", linestyle="None",
...
def get_train_test_data(data: pd.DataFrame, train_start_date: str, test_start_date: str,
    ...
    train_cases = np.array(train_df[data_observed_variables[1]].astype("float")) / data_total_population
    train_timepoints = np.array(train_df.index.astype("float"))

    test_df = data[(data[data_observed_variables[0]] >= test_start_date) & (data[data_observed_variables[0]] < test_end_date)]
    test_cases = np.array(test_df[data_observed_variables[1]].astype("float")) / data_total_population
    ...
    for time, row in train_df.iterrows():
        ...
        if row[data_observed_variables[2]] > 0:
            row_dict["Hospitalizations"] = row[data_observed_variables[2]] / data_total_population
...
def plot_intervened_with_data(intervened_model, model_num, time_points, sample_dict, model_population=None, hosp_ratio=None):
    ...
    ax = plot_predictive(new_model_solns, time_points, ax=setup_ax(),
                         title=f"Intervened Model Output - Model {model_num}", color="blue",
                         label="Intervened Model Prior Forecasts")
    ax = plot_observations(sample_dict['model' + str(model_num)]['Cases'][0], time_points, ax=ax, color="black",
...
def ensemble_calibration(models, synth_data_df, start_states, num_samples,
    ...
    inferred_parameters = calibrate_ensemble(ensemble, train_data, num_iterations=num_iterations, verbose=True,
                                             verbose_every=10, autoguide=autoguide)
    calibrated_solution = sample_ensemble(ensemble, timepoints=all_timepoints, num_samples=50,
