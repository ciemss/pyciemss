...
def seq_id_suffix(df):
    ...
    field = df.columns[0]
    return df.assign(**{"index": [maybe_extend(e) for e in df[field]]}).set_index(
        "index"
    )
...
def load_sim_result(results_file, petrisource, orient="wide") -> pd.DataFrame:
    ...
    if orient not in ["wide", "long"]:
        raise ValueError(
            f"Invalid 'orient' passed {orient}. Only 'wide' and 'narrow' accepted."
        )
...
def load(petrisource=None, uniquer=seq_id_suffix) -> nx.MultiDiGraph:
    ...
    input_edges = [
        *zip(
            inputs["is"].map(lambda v: states.index[v - 1]),
    ...
    output_edges = [
        *zip(
            outputs["ot"].map(lambda v: transitions.index[v - 1]),
...
def draw_petri(G: nx.MultiDiGraph, ax=None) -> None:
    ...
    types = {t: [v[0] for v in nodes] for t, nodes in groupby(types, _k)}
...
def natural_order(G):
    ...
    states = [n for n, data in G.nodes(data=True) if data["type"] == "state"]
    return {n: i for i, n in enumerate(states)}


def add_state_indicies(
    G, ordering_principle=natural_order, overwrite=False, in_place=False
):
    """
...
def natural_degradation(
    params: Dict[str, T], states: Tuple[T, ...], t: T
) -> Tuple[T, ...]:
    raise NotImplementedError  # TODO


@register_template("NaturalConversion")
def natural_conversion(
    params: Dict[str, T], t: T, states: Tuple[T, ...]
) -> Tuple[T, ...]:
    # e.g. (I, R) -> (I - 1, R + 1)
    rate = params["gamma"]
...
def grouped_controlled_conversion(
    params: Dict[str, T], t: T, states: Tuple[T, ...]
) -> Tuple[T, ...]:
    raise NotImplementedError  # TODO
...
def petri_to_ode(
    ...
    def dX_dt(t: T, state: Tuple[T, ...]) -> Tuple[T, ...]:
        ...
        derivs = {k: 0.0 for k in state2ind}
        for node, data in G.nodes(data=True):
            if data["type"] == "transition":
                ...
                for i, name in enumerate(
                    sorted(node_input_names, key=lambda k: state2ind[k])
                ):
        ...
        return order_state(
            G,
            **{
                name: deterministic(f"d{name}_dt", dx_dt)
                for name, dx_dt in derivs.items()
            },
        )
...
def order_state(G: nx.MultiDiGraph, **states: T) -> Tuple[T, ...]:
    ...
    return tuple(
        states[name] for name in sorted(states.keys(), key=lambda k: state2ind[k])
    )


def unorder_state(G: nx.MultiDiGraph, *states: T) -> Dict[str, T]:
    ind2state = {
        data["state_index"]: node
        for node, data in G.nodes(data=True)
        if data["type"] == "state"
    }
...
def duplicate_petri_net(G: nx.MultiDiGraph, *, suffix: str = "cf") -> nx.MultiDiGraph:
    ...
    G2.add_nodes_from((f"{node}_{suffix}", data) for node, data in G.nodes(data=True))
    G2.add_edges_from(
...
def intervene_petri_net(
    G: nx.MultiDiGraph, **interventions: Optional[nx.MultiDiGraph],
) -> nx.MultiDiGraph:
    ...
    for name, fragment in interventions.items():
        if fragment is None:
            ...
            assert set(
                v
                for v, data in fragment.nodes(data=True)
                if data["type"] == "transition"
