...
class PetriNetODESystem(DynamicalSystem):
    ...
    def reset(self) -> None:
        ...
        """
        self._observation_var_names: List[str] = []
        self._static_events: List[StaticEvent] = []
        self._dynamic_stop_events: List = []
        self._observation_indices: Dict = {}
        self._observation_values: Dict = {}
        self._observation_indices_and_values_are_set_up: bool = False
    ...
    def load_event(self, event: Event) -> None:
        ...
        if isinstance(event, StaticEvent):
            ...
            bisect.insort(self._static_events, event)  # type: ignore
        else:
    ...
    @pyro.nn.pyro_method
    def observation_model(self, solution: Dict[str, Tensor], var_name: str) -> None:
        """
    ...
    def static_parameter_intervention(self, parameter: str, value: Tensor):
        """
    ...
    def get_solution(self, method="dopri5") -> Solution:
        ...
        for start, stop in bound_pairs:
            first_event = self._static_events[start]

            if isinstance(first_event, StaticParameterInterventionEvent):
                # Apply the intervention
                self.static_parameter_intervention(
                    first_event.parameter,
                    first_event.value,
                )
        ...
        solution_dict = {v: solution[i] for i, v in enumerate(self.var_order.keys())}
...
class MiraPetriNetODESystem(PetriNetODESystem):
    ...
    def deriv(self, t: Time, state: State) -> StateDeriv:
        ...
        derivs = {k: torch.tensor(0.0) for k in states}
        ...
        for transition in self.G.transitions.values():
            ...
            if len(transition.control) > 0:
                flux = (
                    flux
                    * sum([states[k] for k in transition.control])
                    / population_size
                )
        ...
        return tuple(torch.as_tensor(derivs[v]) for v in self.var_order.values())
    ...
    def param_prior(self):
        for param_info in self.G.parameters.values():
            ...
            elif isinstance(param_value, (int, float, numpy.ndarray, Tensor)):
