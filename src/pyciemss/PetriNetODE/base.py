...
class PetriNetODESystem(DynamicalSystem):
    ...
    def _setup_observation_indices_and_values(self):
        ...
        if not self._observation_indices_and_values_are_set_up:
            ...
            for var_name in self._observation_var_names:
                self._observation_indices[var_name] = [
                    i
                    for i, event in enumerate(self._static_events)
                    if isinstance(event, ObservationEvent)
                    and var_name in event.observation.keys()
                ]
                self._observation_values[var_name] = torch.stack(
                    [
                        self._static_events[i].observation[var_name]
    ...
    def _remove_static_events(self, event_class) -> None:
        ...
        """
        self._static_events = [
            event for event in self._static_events if not isinstance(event, event_class)
        ]
        self._observation_indices_and_values_are_set_up = False
    ...
    def observation_model(
        self, solution: Dict[str, torch.Tensor], var_name: str
    ) -> None:
        """
    ...
    def get_solution(self, method="dopri5") -> Solution:
        ...
        assert isinstance(
            self._static_events[0], StartEvent
        ), "Please initialize the model before sampling."

        # Load initial state
        initial_state = tuple(
            self._static_events[0].initial_state[v] for v in self.var_order.keys()
        )
        ...
        bound_indices = (
            ...
            + [
                i
                for i, event in enumerate(self._static_events)
                if isinstance(event, StaticParameterInterventionEvent)
            ]
            + [len(self._static_events)]
        )
        ...
        for start, stop in bound_pairs:
            if isinstance(self._static_events[start], StaticParameterInterventionEvent):
                ...
                self.static_parameter_intervention(
                    self._static_events[start].parameter,
                    self._static_events[start].value,
                )
            ...
            local_solution = odeint(
                self.deriv, initial_state, local_tspan, method=method
            )
    ...
    @pyro.nn.pyro_method
    def add_observation_likelihoods(
        self, solution: Solution, observation_model=None
    ) -> None:
        """
        ...
        for var_name in self._observation_var_names:
            ...
            filtered_solution = {
                v: solution[observation_indices] for v, solution in solution.items()
            }
            if hasattr(self, "compile_observables_p") and self.compile_observables_p:
                for observable in self.compiled_observables:
                    filtered_solution[observable] = torch.squeeze(
                        self.compiled_observables[observable](**filtered_solution),
                        dim=-1,
                    )
    ...
    def log_solution(self, solution: Solution) -> Solution:
        ...
        logged_solution = {
            v: pyro.deterministic(f"{v}_sol", solution[logging_indices])
            for v, solution in solution.items()
        }
...
class MiraPetriNetODESystem(PetriNetODESystem):
    ...
    def __init__(
        ...
        if compile_rate_law_p:
            self.G.parameters = {
                param: value
                for param, value in self.G.parameters.items()
        ...
        if all(
            var.data.get("initial_value", None) is not None
        ...
        for param_info in G.parameters.values():
            ...
            if param_distribution is not None:
                ...
                if param_value is None:
                    ...
                    raise ValueError(
                        f"Parameter {get_name(param_info)} has value {param_value} of type {type(param_value)} which is not supported."
                    )
    ...
    def compile_observables(self) -> Dict[str, SymPyModule]:
        ...
        return {
            observable_id: SymPyModule(
                expressions=[
                    self.extract_sympy(symbolic_observable.observable.expression)
                ]
            )
            for observable_id, symbolic_observable in self.G.observables.items()
        }

    def compile_rate_law(
        self,
    ) -> Callable[[float, Tuple[torch.Tensor]], Tuple[torch.Tensor]]:
        ...
        numeric_derivs = SymPyModule(
            expressions=[symbolic_derivs[get_name(k)] for k in self.var_order.values()]
        )
        return numeric_derivs

    def extract_sympy(
        self, sympy_expr_str: mira.metamodel.templates.SympyExprStr
    ) -> sympy.Expr:
    ...
    def create_var_order(self) -> dict[str, int]:
        ...
        return collections.OrderedDict(
            (get_name(var), var)
            for var in sorted(self.G.variables.values(), key=get_name)
        )
    ...
    @classmethod
    def _from_template_model(
        cls, model_template: mira.metamodel.TemplateModel, **kwargs
    ):
    ...
    def _from_json(cls, model_json: dict, **kwargs):
        return cls.from_mira(
            mira.metamodel.TemplateModel.from_json(model_json), **kwargs
        )
    ...
    @classmethod
    def from_askenet(
        cls, model: mira.modeling.Model, **kwargs
    ...
    def _from_json(cls, model_json: dict, **kwargs):
        ...
        if "templates" in model_json:
            ...
            return cls.from_askenet(
                petrinet.template_model_from_askenet_json(model_json), **kwargs
            )
        elif "regnet" in model_json["header"]["schema"]:
            return cls.from_askenet(
                regnet.template_model_from_askenet_json(model_json), **kwargs
            )
    ...
    def deriv(self, t: Time, state: State) -> StateDeriv:
        if self.compile_rate_law_p:
            ...
            # Get the parameters
            parameters = {
                get_name(param_info): getattr(self, get_name(param_info))
        ...
        for transition in self.G.transitions.values():
            ...
            if len(transition.control) > 0:
                flux = (
                    flux
                    * functools.reduce(
                        operator.add, [states[k] for k in transition.control]
                    )
                    / population_size ** len(transition.control)
                )
        ...
        for param_info in self.G.parameters.values():
            ...
            if isinstance(param_value, torch.nn.Parameter):
                ...
                setattr(
                    self,
                    param_name,
                    pyro.deterministic(param_name, torch.as_tensor(param_value)),
                )
            else:
...
class ScaledNormalNoisePetriNetODESystem(MiraPetriNetODESystem):
    ...
    def observation_model(self, solution: Solution, var_name: str) -> None:
        ...
        scale = self.noise_scale * torch.maximum(
            mean, torch.as_tensor(0.005 * self.total_population)
        )
    ...
    def observation_model(self, solution: Solution, var_name: str) -> None:
        ...
        if self.parameterized_by_pseudocount:
            pyro.sample(
                var_name,
                ScaledBeta(
                    mean, self.total_population, pseudocount=self.pseudocount
                ).to_event(1),
            )
        else:
            scale = self.noise_scale * torch.maximum(
            ...
            pyro.sample(
                var_name,
                ScaledBeta(mean, self.total_population, scale=scale).to_event(1),
            )
